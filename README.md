...
- `callers`: needs a live holding — "no holding" means acquire first.
  ...
  (rust default, also python/go). Warm calls skip the reload
  (measured ~2s on deepFunc's own tree, milliseconds on small
  workspaces). Empty report is an answer, not an error.
